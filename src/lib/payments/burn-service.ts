...
export class BurnService {
  ...
  async processPendingBurns(): Promise<{
    ...
    console.log(
      `Plan: Burn ${burnAmount.toString()} tokens for ${pendingTransactions.length} transactions.`
    );
    ...
    if (signature) {
      await prisma.tokenTransaction.updateMany({
        where: { id: { in: pendingTransactions.map((tx: any) => tx.id) } },
...
export async function runBurnService() {
  ...
  if (!treasury || !mint) {
    console.error(
      'CRITICAL: Burn service requires TREASURY_WALLET and TOKEN_MINT environment variables.'
    );
    return null;
  }

...
import { estimateWorkflowCost, estimateWorkflowRunCost, getDefaultModelCost } from './estimator';
...
it('estimates workflow cost with steps', () => {
  ...
  expect(
    result.disclaimer.includes('external') || result.disclaimer.includes('minimum')
  ).toBeTruthy();
});

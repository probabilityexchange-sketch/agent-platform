...
export async function extendContainer(
  ...
  return await prisma.$transaction(async (tx: any) => {
    ...
    const newExpiresAt = new Date(container.expiresAt.getTime() + additionalHours * 60 * 60 * 1000);
...
export async function getContainerLogs(dockerId: string, tail: number = 100): Promise<string> {
  const bridge = getComputeBridge();
  if (bridge) {

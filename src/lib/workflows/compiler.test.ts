...
it('compiles a scheduled monitoring workflow with GitHub Actions preference', () => {
  const plan = compileWorkflowPlan(
    'Monitor pump.fun opportunities every hour and alert me in Telegram'
  );
  ...
  expect(
    plan.toolRecommendations.some(item => item.suggestedApproach === 'GitHub Actions')
  ).toBeTruthy();
});

it('marks financial workflows as needing policy confirmation', () => {
  const plan = compileWorkflowPlan(
    'When a token breaks out, buy it and send the fill summary to Telegram'
  );
  ...
  expect(
    plan.approvals.some(approval => approval.reason.toLowerCase().includes('financial'))
  ).toBeTruthy();
});

it('recommends GitHub Actions instead of cron jobs', () => {
  const plan = compileWorkflowPlan(
    'Replace my cron job with a workflow that checks new GitHub issues daily and posts a summary'
  );

  expect(
    plan.toolRecommendations.some(
      item => item.currentApproach.includes('Cron') || item.currentApproach.includes('scheduled')
    )
  ).toBeTruthy();
  expect(
    plan.toolRecommendations.some(item => item.suggestedApproach === 'GitHub Actions')
...
it('builds GitHub integration recommendation for manual repo checking', () => {
  ...
  expect(
    recommendations.some(item => item.suggestedApproach === 'GitHub integration')
  ).toBeTruthy();
  expect(
    recommendations.some(item => item.reason.includes('structured repository state'))
  ).toBeTruthy();
});
...
it('builds CoinMarketCap recommendation for generic crypto lookups', () => {
  ...
  expect(
    recommendations.some(item => item.reason.includes('structured crypto prices'))
  ).toBeTruthy();
});
...
it('compiler includes multiple truthful substitution patterns when applicable', () => {
  ...
  expect(
    plan.toolRecommendations.some(item => item.suggestedApproach === 'GitHub integration')
  ).toBeTruthy();
  expect(
    plan.toolRecommendations.some(item => item.suggestedApproach === 'CoinMarketCap')
  ).toBeTruthy();
});

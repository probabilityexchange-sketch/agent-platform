...
const FINANCIAL_REGEX =
  /\b(buy|sell|swap|trade|transfer|send money|payment|pay|credit card|wallet|token|sol|usdc|exchange)\b/i;
const WRITE_ACTION_REGEX =
  /\b(create|update|delete|send|post|publish|deploy|open|submit|write|commit|push)\b/i;
const WORKFLOW_REGEX =
  /\b(workflow|automation|automate|recurring|repeat|reusable|schedule|scheduled|every|daily|weekly|hourly|monitor|watch|track|alert|notify|save this|run this regularly|cron|github actions)\b/i;
...
export function looksLikeWorkflowRequest(message: string): boolean {
  ...
  const hasAutomationShape =
    /\b(if|when|whenever)\b/.test(normalized) &&
    /\b(alert|notify|run|check|monitor|track)\b/.test(normalized);
...
function buildDefaultSteps(
  message: string,
  triggerType: WorkflowPlan['trigger']['type']
): string[] {
...
function buildSteps(message: string, trigger: WorkflowPlan['trigger']): WorkflowStep[] {
  ...
  const rawSteps =
    descriptions.length > 0 ? descriptions : buildDefaultSteps(message, trigger.type);
...
function buildApprovals(message: string, steps: WorkflowStep[]): WorkflowPlan['approvals'] {
  ...
  const writeStepIds = steps
    .filter(step => step.requiresApproval && step.kind !== 'financial')
    .map(step => step.id);
  ...
  if (FINANCIAL_REGEX.test(message) || financialStepIds.length > 0) {
    approvals.push({
      type: 'required',
      reason:
        'Financial actions must stay behind explicit approval, hard caps, and audit logging before execution.',
  ...
  if (WRITE_ACTION_REGEX.test(message) || writeStepIds.length > 0) {
    approvals.push({
      type: 'recommended',
      reason:
        'This workflow appears to write to an external system, so scopes and approval boundaries should be explicit.',
...
): WorkflowPlan['toolRecommendations'] {
...
function buildOpenQuestions(
  ...
  if (
    (trigger.type === 'monitor' || trigger.type === 'event') &&
    !/\b(if|when|whenever|above|below|crosses|threshold|opportunity)\b/i.test(normalized)
  ) {
  ...
  if (
    /\b(alert|notify|send|post|report)\b/i.test(normalized) &&
    !/\b(slack|telegram|email|gmail|notion|github)\b/i.test(normalized)
  ) {
    questions.add('Where should updates or alerts be delivered?');
  }
  if (FINANCIAL_REGEX.test(normalized)) {
    questions.add(
      'What transaction caps and allowlists should apply before any money movement is enabled?'
    );
  }
...
function buildRiskNotes(message: string, steps: WorkflowStep[]): string[] {
  ...
  if (FINANCIAL_REGEX.test(message)) {
    notes.add(
      'Do not execute financial actions without explicit approval, caps, and audit logging.'
    );
  }
  if (steps.some(step => step.requiresApproval)) {
    notes.add(
      'External write actions should remain behind explicit approval until policy rules are in place.'
    );
  }
  if (/\b(monitor|watch|track|scan)\b/i.test(message)) {
    notes.add(
      'Monitoring workflows need clear thresholds to avoid noisy alerts or unnecessary runs.'
    );
  }
  return [...notes];
}
...
export function compileWorkflowPlan(message: string): WorkflowPlan {
  ...
  const requiresFinancialControls = approvals.some(approval =>
    approval.reason.toLowerCase().includes('financial')
  );

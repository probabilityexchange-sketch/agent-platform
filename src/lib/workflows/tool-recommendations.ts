...
const TOOL_RECOMMENDATION_RULES: ToolRecommendationRule[] = [
  {
    ...
    matches: ({ normalized, trigger }) => {
      return (
        normalized.includes('cron') || trigger.type === 'schedule' || trigger.type === 'monitor'
      );
    },
    build: ({ normalized }) => ({
      ...
      reason:
        'GitHub Actions is the preferred scheduling path here because it gives repo-native logs and a safer recurring runner than ad-hoc cron.',
    }),
  },
  {
    ...
    matches: ({ normalized }) => {
      ...
      const manualChecking =
        /\b(check|review|look up|monitor|watch|track|status|scan|manually)\b/i.test(normalized);
    ...
    build: () => ({
      ...
      reason:
        'A GitHub integration gives structured repository state for issues, pull requests, and commits instead of relying on manual checking or generic summaries.',
    }),
  },
  {
    id: 'generic-crypto-lookup-coinmarketcap',
    matches: ({ normalized }) => {
      const mentionsCrypto =
        /\b(crypto|token|coin|market cap|price|volume|ranking|rankings)\b/i.test(normalized);
      const genericLookup =
        /\b(lookup|look up|check|monitor|track|research|find|latest|price|prices)\b/i.test(
          normalized
        );
      const isFinancialAction = /\b(buy|sell|swap|trade|transfer|send money|payment|pay)\b/i.test(
        normalized
      );
    ...
    build: () => ({
      ...
      reason:
        'CoinMarketCap provides structured crypto prices, rankings, and market metadata, which is more reliable for this kind of lookup than a generic research path.',
    }),
  },
];

export function buildToolRecommendationsFromHeuristics(
  message: string,

...
export function buildStorageKey(userId: string, agentSlug: string): string {
  return createHash('sha256').update(`${userId}:${agentSlug}`).digest('hex').slice(0, 16);
}
...
export async function hasSnapshot(storageKey: string): Promise<boolean> {
  ...
  const { data, error } = await supabase.storage.from(STORAGE_BUCKET).list(storageKey);
 ...
 */
export async function getSnapshotDownloadUrl(storageKey: string): Promise<string | null> {
  const supabase = getSupabaseAdmin();
  const path = getSnapshotPath(storageKey);
 ...
 */
export async function getSnapshotUploadUrl(storageKey: string): Promise<string | null> {
  const supabase = getSupabaseAdmin();
  const path = getSnapshotPath(storageKey);

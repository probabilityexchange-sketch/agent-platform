...
const DEFAULT_DESCRIPTION =
  'Reusable capability that extends how the agent reasons and executes tasks.';
...
function categorizeSkill(slug: string, description: string): string {
  ...
  if (
    /(react|ux|design|frontend|presentation|slides|spreadsheet|pdf|pptx|xlsx|doc)/.test(searchable)
  ) {
    return 'Product';
  }
  return 'General';
...
async function readSkillFromRoot(
  ...
  try {
    ...
    const description =
      readFrontmatterString(frontmatter, 'description') || extractDescription(content);
...
export async function loadSkillCatalog(): Promise<SkillSummary[]> {
  ...
  for (const root of getAbsoluteRoots()) {
    ...
    const docs = await Promise.all(
      slugs.map(slug => readSkillFromRoot(root.source, root.absoluteRoot, slug))
    );

...
export function ResourceMonitor({ containerId, pollIntervalMs = 5000 }: ResourceMonitorProps) {
  ...
  if (error) {
    return <div className="text-xs text-yellow-400/70 px-1">Stats unavailable</div>;
  }

  if (!stats) {
    return <div className="text-xs text-white/40 animate-pulse px-1">Loading stats…</div>;
  }

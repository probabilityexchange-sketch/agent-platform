...
export function AgentCard({ agent, onLaunch, userTier = 'FREE' }: AgentCardProps) {
  ...
  return (
    <div
      className={`group relative bg-card/40 hover:bg-card border border-border hover:border-primary/30 rounded-2xl p-6 transition-all duration-300 flex flex-col h-full ${isLocked ? 'grayscale opacity-60' : 'hover:shadow-2xl hover:shadow-primary/5'}`}
    >
      ...
      <div className="space-y-2 mt-auto">
        <Link
          ...
          className={`w-full inline-flex items-center justify-center px-4 py-2.5 rounded-xl font-bold text-sm transition-all duration-300 ${
            isLocked

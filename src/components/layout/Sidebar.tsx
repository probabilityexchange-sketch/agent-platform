...
export function Sidebar() {
  ...
  const sidebar = (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar space-y-6">
        ...
        <div>
          <h3 className="mb-2 px-3 text-xs font-semibold text-muted-foreground">Advanced tools</h3>
          <nav className="space-y-1">{advancedNavItems.map(renderNavItem)}</nav>
        </div>
      </div>
  ...
  return (
    <>
      ...
      {/* Mobile sidebar */}
      <aside
        className={`lg:hidden fixed inset-y-0 left-0 z-50 w-64 bg-card border-r border-border p-4 flex flex-col transform transition-transform duration-300 ${

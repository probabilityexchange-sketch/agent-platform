...
export function WorkflowScheduleCard({ schedule, onAction }: WorkflowScheduleCardProps) {
  const getStatusTheme = (status: WorkflowScheduleStatus) => {
    switch (status) {
      ...
      case 'paused':
        return { label: 'Paused', color: 'text-amber-500 bg-amber-500/10 border-amber-500/20' };
      case 'blocked':
        return { label: 'Blocked', color: 'text-rose-500 bg-rose-500/10 border-rose-500/20' };
      default:
        return { label: status, color: 'text-muted-foreground bg-muted/20 border-border' };
    }
  };
  ...
  const getSyncTheme = (state: WorkflowScheduleDeploymentState) => {
    switch (state) {
      ...
      case 'pending_manual_sync':
        return {
          label: 'Pending first GitHub sync',
      ...
      case 'blocked':
        return { label: 'Sync blocked', color: 'text-rose-400 border-rose-500/30 bg-rose-500/5' };
      default:
        return { label: state, color: 'text-muted-foreground border-border bg-muted/10' };
    }
  };
    ...
    switch (schedule.deploymentState) {
      ...
      case 'needs_resync':
        return {
          ...
          body:
            schedule.deploymentReason ||
            'The workflow changed after scheduling. Update the GitHub workflow file and secrets so the deployed definition matches the current workflow.',
      ...
      case 'blocked':
        return {
          ...
          body:
            schedule.deploymentReason ||
            'The backend is blocking schedule deployment, so GitHub deployment should not proceed yet.',
    ...
    <div className="my-4 border border-border/60 bg-card/40 rounded-2xl overflow-hidden text-sm shadow-lg">
      ...
      <div className="px-6 py-4 bg-muted/30 border-b border-border/60 flex items-center justify-between">
        ...
        <div className="flex flex-col items-end gap-1.5">
          <span
            className={`text-[9px] font-black uppercase px-2.5 py-0.5 rounded-full border tracking-widest ${statusTheme.color}`}
          >
            {statusTheme.label}
          </span>
          <span
            className={`text-[8px] font-black uppercase px-2 py-0.5 rounded border tracking-[0.1em] ${syncTheme.color}`}
          >
            {syncTheme.label}
          </span>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        {deploymentGuidance && (
          <div className={`p-4 border rounded-2xl ${deploymentGuidance.tone}`}>
            <p className="text-[10px] font-black uppercase tracking-[0.15em] mb-2">
              {deploymentGuidance.title}
            </p>
            <p
        ...
        {schedule.status === 'blocked' && schedule.deploymentState !== 'blocked' && (
          <div className="p-4 bg-rose-500/5 border border-rose-500/20 rounded-2xl">
            <p className="text-[10px] text-rose-500 font-black uppercase tracking-[0.15em] mb-2">
              App Schedule Blocked
            </p>
            <p className="text-xs text-rose-100/80 italic leading-relaxed font-medium">
              {schedule.deploymentReason ||
                schedule.lastError ||
                'Activation refused by safety policy. Check workflow readiness and policy status.'}
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-black/20 p-3 rounded-2xl border border-white/5">
            <p className="text-[9px] text-muted-foreground uppercase tracking-widest font-black mb-1 opacity-60 italic">
              Last Fire
            </p>
            <p className="text-xs font-black text-foreground/90">
              {formatTime(schedule.lastTriggeredAt)}
            </p>
          </div>
          <div className="bg-black/20 p-3 rounded-2xl border border-white/5">
            <p className="text-[9px] text-muted-foreground uppercase tracking-widest font-black mb-1 opacity-60 italic">
            ...
            <p className="text-xs font-black text-emerald-400">
              {formatTime(schedule.lastSuccessfulAt)}
            </p>
          </div>
        </div>
        ...
        {schedule.consecutiveFailures > 0 && (
          <div className="flex items-center gap-3 text-rose-400 text-[10px] font-black bg-rose-500/5 p-3 rounded-xl border border-rose-500/10 uppercase tracking-widest italic">
            <span className="text-lg animate-pulse">⚠️</span> {schedule.consecutiveFailures}{' '}

...
export function WorkflowRunCard({ run, onAction }: WorkflowRunCardProps) {
  ...
  return (
    <div
      className={`my-4 border rounded-2xl overflow-hidden text-sm shadow-md transition-all ${
    ...
    >
      ...
      <div className="px-5 py-3 border-b border-border/60 flex items-center justify-between bg-muted/20">
        <div className="flex items-center gap-3">
          ...
          <div
            className={`flex items-center gap-1.5 px-3 py-0.5 rounded-full border text-[10px] font-black uppercase tracking-widest ${theme.color}`}
          >
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-muted-foreground uppercase tracking-[0.2em] font-black opacity-60">
            Source:
          </span>
          <span className="text-[10px] text-foreground font-black uppercase italic tracking-tight">
            {run.triggerSource}
          </span>
        </div>
      </div>
      ...
      <div className="p-5 space-y-4">
          ...
          <div className="p-4 bg-amber-500/5 border border-amber-500/20 rounded-2xl shadow-inner">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-[10px] text-amber-500 font-black uppercase tracking-[0.15em]">
                Run Blocked Pending Review
              </p>
            </div>
            <p className="text-xs text-amber-100/80 italic leading-relaxed font-medium">
              {run.blockedReason ||
                'The backend blocked this run. Review the policy or approval requirement before retrying.'}
            </p>
        ...
        {isFailed && run.lastError && (
          <div className="p-4 bg-rose-500/5 border border-rose-500/20 rounded-2xl">
            <p className="text-[10px] text-rose-500 font-black uppercase tracking-widest mb-2">
              Error Diagnostic
            </p>
          ...
          <div className="grid grid-cols-2 gap-4 pt-2 border-t border-border/40">
            {run.estimatedTokens !== null && run.estimatedTokens !== undefined && (
              <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-3">
                <p className="text-[9px] text-blue-400 uppercase tracking-widest font-black mb-1">
                  Estimated
                </p>
                <p className="text-sm font-black text-blue-200">
                  {run.estimatedTokens.toLocaleString()} tokens
                </p>
              </div>
            )}
            {run.actualTokens !== null &&
              run.actualTokens !== undefined &&
              run.status === 'completed' && (
                <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-3">
                  ...
                  <p className="text-sm font-black text-emerald-200">
                    {run.actualTokens.toLocaleString()} tokens
                  </p>
        ...
        {run.status === 'running' && (
          <div className="pt-2 px-1">
            <div className="flex justify-between items-center mb-1.5">
              <span className="text-[10px] text-primary font-black uppercase tracking-widest animate-pulse italic">
                In Progress...
              </span>

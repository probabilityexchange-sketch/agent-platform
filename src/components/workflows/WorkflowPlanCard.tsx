...
export function WorkflowPlanCard({ plan, onSave, isSaved = false }: WorkflowPlanCardProps) {
  ...
  const readinessLabel =
    plan.readiness === 'needs_policy_confirmation'
      ? isSaved
        ? 'Saved, pending policy confirmation'
        : 'Needs policy confirmation'
      : isSaved
        ? 'Saved, needs workflow details'
  ...
  return (
    <div
      className={`my-4 border rounded-2xl overflow-hidden text-sm shadow-lg transition-all ${
    ...
    >
      ...
      <div className="p-6 space-y-6">
        <div>
          <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-black mb-2 opacity-60">
            Objective
          </p>
          ...
          <ul className="space-y-2">
            {plan.readiness === 'needs_policy_confirmation' && (
              <li className="text-xs text-amber-200/80 flex gap-2 font-medium">
                <span className="text-amber-500">•</span> Saving this plan does not bypass runtime
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-black/20 p-3 rounded-2xl border border-white/5">
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-black mb-1.5 opacity-60">
              Trigger
            </p>
          ...
          <div className="bg-black/20 p-3 rounded-2xl border border-white/5">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`w-2 h-2 rounded-full ${plan.guardrails.requiresExplicitApproval ? 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]' : 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]'}`}
              />
              <span className="text-xs font-black uppercase tracking-tight">
                {plan.guardrails.requiresExplicitApproval
                  ? 'Approval required'
        ...
        {plan.toolRecommendations.length > 0 && (
          <div className="p-4 bg-sky-500/5 border border-sky-500/20 rounded-2xl">
            ...
            <p className="text-xs text-sky-100/80 leading-relaxed font-medium mb-3">
              These are recommendations only. They describe potentially better tool paths for this
              workflow and do not mean any tool has been connected, selected, or run.
            </p>
            <div className="space-y-3">
              {plan.toolRecommendations.map((recommendation, index) => (
                <div
                  key={`${recommendation.currentApproach}-${recommendation.suggestedApproach}-${index}`}
                  className="rounded-2xl border border-sky-500/15 bg-black/20 p-3"
                >
        ...
        <div>
          ...
          <div className="space-y-2.5">
            {plan.steps.slice(0, expanded ? undefined : 2).map((step, i) => (
              <div
                key={step.id}
                className="flex gap-4 items-start p-3 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-colors group"
              >
                <span className="text-[10px] opacity-30 font-black mt-1 group-hover:opacity-60 transition-opacity italic">
                  0{i + 1}
                </span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {step.requiresApproval && (
                      <span className="text-[8px] bg-amber-500/10 text-amber-500 px-1.5 py-0.5 rounded-full border border-amber-500/20 font-black tracking-tighter">
                        GATE
                      </span>
                    )}
                  </div>

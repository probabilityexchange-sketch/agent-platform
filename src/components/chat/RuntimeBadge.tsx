...
export function RuntimeBadge({ agentId, sessionId }: { agentId: string; sessionId?: string }) {
  ...
  const handleRetry = async () => {
    setLoading(true);
    setStatus(prev =>
      prev ? { ...prev, state: 'starting' } : { state: 'starting', runtimeTarget: 'shared' }
    );
    try {
      ...
      <div className="flex flex-col gap-2 p-4 bg-red-50/50 border border-red-200 rounded-2xl max-w-sm">
        ...
        {showDetails && (
          <div className="mt-3 p-3 bg-red-100/30 rounded-lg border border-red-200/50 text-[10px] font-mono text-red-800 space-y-1 overflow-x-auto">
            <p>
              <span className="opacity-50 uppercase tracking-tighter">Request ID:</span>{' '}
              {status.lastErrorRequestId}
            </p>
            <p>
              <span className="opacity-50 uppercase tracking-tighter">Status:</span>{' '}
              {status.lastErrorStatusCode}
            </p>
            <p>
              <span className="opacity-50 uppercase tracking-tighter">Category:</span>{' '}
              {status.lastErrorCategory}
            </p>
            <p>
              <span className="opacity-50 uppercase tracking-tighter">Error:</span>{' '}

...
export function ChatWindow({
  ...
  const handleWorkflowAction = useCallback(
    async (workflowId: string, action: string, data?: any) => {
      if (action === 'save') {
        try {
          await sendMessage(
            {
        ...
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center p-8">
            ...
            <p className="mt-2 text-muted-foreground max-w-xl leading-relaxed">
              Ask Randi to research, summarize, draft, plan, or help you take an action. Paste notes
              or a link if you have them. If a connected tool is needed, you can review the request
              before anything sensitive runs.
            </p>
            ...
            <div className="mt-6 max-w-xl rounded-xl border border-border/60 bg-background/60 p-4 text-left">
              ...
              <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
                Randi only pauses when a tool request needs review. You will see the app, requested
                action, likely effect, and technical details before choosing whether to continue.
              </p>
            </div>
          </div>
        )}
      ...
      <div className="p-4 border-t border-border/60 bg-card/60">
        {(chatError || localError) && (
          <div className="mb-3 flex items-start justify-between gap-3 rounded-xl bg-red-500/15 border border-red-500/30 px-4 py-3">
            <p className="text-sm text-red-300">
              {(chatError as any)?.message || localError || 'An error occurred'}
            </p>
            <button
              onClick={() => regenerate()}
        ...
        <div className="mb-3 grid gap-3 rounded-xl border border-border/60 bg-background/60 px-4 py-3 sm:grid-cols-2">
          <div>
            ...
            <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
              Say what you want back, paste the source material, and mention any constraints or tone
              you care about.
            </p>
          </div>
          <div>
            ...
            <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
              Planning and drafting can happen immediately. Sensitive tool actions pause here so you
              can approve or decline them.
            </p>
          </div>
        </div>
        <ChatInput onSend={handleSendMessage} disabled={isLoading} initialValue={initialDraft} />
      </div>
    </div>
  );
}

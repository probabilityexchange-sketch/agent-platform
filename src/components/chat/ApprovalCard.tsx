...
function inferRisk(toolName: string) {
  ...
  if (/(DELETE|REMOVE|CANCEL|TRANSFER|PAY|EXECUTE|PURCHASE|WITHDRAW)/.test(upper)) {
    return {
      level: 'High',
      tone: 'rose' as const,
      summary: 'This action may make a harder-to-undo external change.',
    };
  }
  ...
  if (/(CREATE|UPDATE|POST|SEND|WRITE|UPLOAD|PUBLISH|INSERT|PATCH)/.test(upper)) {
    return {
      level: 'Medium',
      tone: 'amber' as const,
      summary: 'This action writes or updates something in the connected app.',
    };
  }
  ...
  if (/(GET|LIST|SEARCH|READ|FETCH)/.test(upper)) {
    return {
      ...
      summary: 'This action mostly reads information from the connected app.',
    };
  }
  ...
  return {
    ...
    summary: 'Review the details before approving the request.',
  };
}
...
export function ApprovalCard({ request, onDecision, decided = 'PENDING' }: ApprovalCardProps) {
  ...
  const summary =
    request.summary?.trim() || request.description?.trim() || `${actionLabel} in ${service}`;
      ...
      >
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-lg font-bold leading-tight text-foreground">{summary}</h3>
            <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{actionSummary}</p>
          </div>
        </div>
        ...
        <div className="bg-background/10 px-6 py-5">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <span
              className={`rounded-full border px-3 py-1.5 text-sm font-medium ${labelTone(risk.tone)}`}
            >
              {risk.level} review
            </span>
          ...
          <div className="mt-5 rounded-2xl border border-border/60 bg-background/30 p-4">
            ...
            <p className="mt-2 text-sm text-foreground/90 leading-relaxed">{request.description}</p>
          </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            ...
            <div className="rounded-2xl border border-border/60 bg-background/30 p-4">
              ...
              <p className="mt-2 text-sm text-foreground/90 leading-relaxed">
                Approve to let Randi continue with this step. Decline to stop this request so
                nothing from this approval runs.
              </p>
            </div>
          </div>

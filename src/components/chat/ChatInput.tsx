...
export function ChatInput({ onSend, disabled, initialValue = '' }: ChatInputProps) {
  ...
  return (
    <div className="space-y-2">
      <div
        className={`flex items-end gap-2 bg-background/50 border rounded-xl p-2 transition-all ${
          isListening
            ? 'border-primary shadow-[0_0_15px_rgba(var(--primary),0.3)] ring-1 ring-primary/50'
      ...
      >
        ...
        <textarea
          ...
          placeholder={
            isListening
              ? 'Listening. Speak your task or context…'
              : 'Describe one task. Example: draft a reply, summarize this link, or plan the next step.'
          }
          rows={1}
          disabled={disabled}
      ...
      <p id={helperId} className="px-1 text-sm text-muted-foreground">
        Press Enter to send. Use Shift+Enter for a new line. Randi will pause in chat if a connected
        tool needs approval.
      </p>
    </div>
  );

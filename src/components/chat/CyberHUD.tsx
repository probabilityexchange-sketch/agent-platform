    ...
    <div className="flex flex-col h-full bg-[#0a0a0f] border-l border-white/5 p-6 relative overflow-hidden">
      ...
      <div className="space-y-8 relative z-10">
        ...
        <div className="relative">
          <div
            className={`absolute -inset-1 rounded-xl blur-md transition-opacity duration-500 ${isActive ? 'bg-primary/20 opacity-100' : 'opacity-0'}`}
          />
          <div className="relative flex items-center gap-4 bg-white/5 border border-white/10 rounded-xl p-4 backdrop-blur-md">
            <div
              className={`w-10 h-10 rounded-lg flex items-center justify-center border transition-colors ${isActive ? 'bg-primary/20 border-primary shadow-[0_0_15px_rgba(var(--primary),0.4)]' : 'bg-white/5 border-white/10'}`}
            >
              <svg
        ...
        <div className="relative">
          <div
            className={`absolute -inset-1 rounded-xl blur-md transition-opacity duration-500 ${isActive ? 'bg-blue-500/10 opacity-100' : 'opacity-0'}`}
          />
          <div className="relative flex items-center gap-4 bg-white/5 border border-white/10 rounded-xl p-4 backdrop-blur-md">
            <div
              className={`w-10 h-10 rounded-lg flex items-center justify-center border transition-colors ${isActive ? 'bg-blue-500/20 border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.2)]' : 'bg-white/5 border-white/10'}`}
            >
              <svg
                className={`w-5 h-5 ${isActive ? 'text-blue-400' : 'text-white/40'}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <div className="overflow-hidden">
              ...
              <div className="text-xs font-semibold text-white/90 truncate max-w-[150px]">
                {modelId.split('/').pop()}
              </div>
            </div>
          </div>
      ...
      <div className="mt-auto mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div
            className={`text-[10px] font-mono ${isActive ? 'text-emerald-400 animate-pulse' : 'text-white/20'}`}
          >

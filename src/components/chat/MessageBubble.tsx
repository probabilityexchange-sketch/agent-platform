  ...
  return (
    <div className="my-2 border border-border bg-muted/30 rounded-lg overflow-hidden text-xs">
      <div className="flex items-center justify-between px-3 py-2 bg-muted/50 border-b border-border">
        ...
        <span className="text-[10px] text-muted-foreground font-mono uppercase opacity-70">
          {toolCallId.slice(-6)}
        </span>
      </div>
...
function useMarkdownComponents(isStreaming: boolean): Components {
  return useMemo<Components>(
    () => ({
      ...
      p({ children }) {
        return (
          <p className="mb-2 last:mb-0 leading-relaxed">
            {children}
            {isStreaming && (
              <span className="inline-block w-0.5 h-4 ml-0.5 bg-white/70 animate-pulse align-middle" />
            )}
          </p>
        );
      },
      ...
      li: ({ children }) => <li className="leading-relaxed text-[0.92em]">{children}</li>,
...
export function MessageBubble({
  ...
  const timestamp =
    !mounted || Number.isNaN(messageDate.getTime())
      ? null
  ...
  return (
    <div className={`group flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-[85%] lg:max-w-[80%] px-4 py-3 rounded-2xl ${
          isUser
            ? `bg-primary text-white rounded-br-md text-sm whitespace-pre-wrap leading-relaxed ${
                message.error ? 'ring-2 ring-red-500/60' : ''
              }`
      ...
      >
        {isUser ? (
          ...
          <div className="text-sm leading-relaxed space-y-2">
            ...
            {isStreaming &&
              !fullText &&
              (!message.parts || message.parts.every(p => p.type !== 'tool-invocation')) && (
        ...
        <div
          className={`flex items-center gap-2 mt-1.5 ${isUser ? 'justify-end' : 'justify-between'}`}
        >

      ...
      <div className="h-full flex items-center justify-center p-8">
        <div className="max-w-md rounded-2xl border border-rose-500/20 bg-rose-500/10 p-6 text-center">
          ...
          <p className="mt-3 text-sm text-rose-200/90">
            {sessionError ||
              'Randi’s session did not initialize correctly. Please retry and I’ll prepare a fresh chat.'}
          </p>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)] max-w-5xl mx-auto p-4 md:p-6 gap-4">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-border bg-background/40 px-3 py-1.5 text-sm font-medium text-muted-foreground">
              {toolsLoading
                ? 'Checking tools…'
                : `${connectedCount} tool${connectedCount === 1 ? '' : 's'} ready`}
            </span>
            <span className="rounded-full border border-border bg-background/40 px-3 py-1.5 text-sm font-medium text-muted-foreground">
              {currentModelLabel}
            </span>
      ...
      <div className="rounded-2xl border border-dashed border-border bg-card/20 p-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Model choice and agent controls live here so the main conversation stays visible
              first.
            </p>
          </div>
          <button
        ...
        {advancedOpen && (
          <div
            id="advanced-chat-controls"
            className="mt-4 rounded-2xl border border-border bg-card/30 p-4 space-y-4"
          >
            <div className="flex flex-wrap gap-2">
              {toolsLoading ? (

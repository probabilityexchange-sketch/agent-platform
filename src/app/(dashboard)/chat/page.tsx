...
const CHAT_START_STEPS = [
  ...
  },
  {
    ...
    description:
      'She can reason, compare options, organize information, and prepare work before using any tool.',
  },
  {
    title: 'Review tool actions when needed',
    description:
      'If a connected app is involved, Randi can pause and ask for approval before anything sensitive runs.',
  },
];

const TRUST_BADGES = [
  'Research, drafts, and plans',
  'Approvals stay reviewable',
  'Automate later when ready',
];
...
export default function ChatHubPage() {
      ...
      .then(data => {
        setConnectedIntegrations(
          (data.integrations || []).filter(
            (integration: IntegrationSummary) => integration.connected
          )
        );
      })
  ...
  const promptChips = useMemo(
    () => buildPromptChips(connectedIntegrations),
    ...
    <div className="max-w-5xl mx-auto px-4 py-12 min-h-[calc(100vh-8rem)]">
      <div className="text-center mb-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <p className="text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
          Start with one real task. Randi can research, draft, plan, and use your connected tools
          when you want her to act.
        </p>
      ...
      <div className="grid gap-4 md:grid-cols-[1.2fr_0.8fr] mb-8 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-150">
          ...
          <ul className="mt-4 space-y-4">
            {CHAT_START_STEPS.map(step => (
              <li key={step.title} className="flex gap-3">
                <span
                  className="mt-1 h-2.5 w-2.5 flex-shrink-0 rounded-full bg-primary/80"
                  aria-hidden="true"
                />
                <div>
        ...
        <div className="rounded-3xl border border-border bg-card/40 p-6">
          ...
          <div className="mt-4 rounded-2xl border border-border/60 bg-background/40 p-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground leading-relaxed">
              Review happens in chat. When approval is needed, you will see the app, requested
              action, likely effect, and any target details the tool provides.
            </p>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center mb-10 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200">
        ...
        <button
          ...
          aria-label={
            activeTab === 'recent' ? 'Show suggested starting prompts' : 'Show recent chats'
          }
        ...
        >
          ...
          {activeTab === 'recent'
            ...
            : `Browse recent chats${sessions.length > 0 ? ` (${sessions.length})` : ''}`}
        </button>
      </div>
          ...
          {sessions.length === 0 ? (
            ...
            sessions.map(session => (
              ...
              >
                ...
                <svg
                  className="w-5 h-5 text-muted-foreground/30 group-hover:text-primary transition-colors"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >

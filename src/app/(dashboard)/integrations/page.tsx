...
function connectionSummary(integration: IntegrationItem) {
  ...
  if (
    integration.connectedStatus === 'INITIATED' ||
    integration.connectedStatus === 'INITIALIZING'
  ) {
...
function IntegrationCard({
  ...
  const suggestedPrompt =
    integration.suggestedPrompt || 'Help me use this connected tool inside chat.';

  return (
    <div
      className={`group relative flex h-full flex-col overflow-hidden rounded-[2rem] border transition-all duration-300 ${
        isConnected
    ...
    >
      ...
      <div className="p-6 pb-4">
        <div className="flex items-start justify-between gap-4">
          <div
            className={`flex h-14 w-14 flex-shrink-0 items-center justify-center overflow-hidden rounded-2xl text-2xl shadow-sm transition-transform group-hover:scale-105 ${
      ...
      <div className="flex-1 px-6 pb-6">
            ...
            <ul className="mt-2.5 space-y-2">
              {capabilities.map(capability => (
                <li key={capability} className="flex items-center gap-2 text-sm text-foreground/80">
                  <div
                    className={`h-1 w-1 rounded-full ${isConnected ? 'bg-emerald-400' : 'bg-primary/40'}`}
                  />
                  {capability}
                </li>
              ))}
        ...
        <div className="mt-6 flex gap-3">
          <button
            ...
            className={`flex-1 rounded-2xl px-4 py-3 text-sm font-bold transition-all disabled:cursor-not-allowed disabled:opacity-40 ${
              isConnected
...
function IntegrationsPageContent() {
  ...
  const categories = useMemo(() => {
    ...
    const extras = Array.from(present)
      .filter(category => !COMPOSIO_CATEGORIES.includes(category as never))
  ...
  return (
    <div className="max-w-6xl space-y-12 pb-20">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-lg text-muted-foreground leading-relaxed">
            Enable Randi to read, draft, and act across your favorite apps. Connections are secure
            and require your approval for sensitive actions.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          <Link
            href="/chat"
            className="inline-flex h-12 items-center justify-center rounded-2xl bg-primary px-6 text-sm font-bold text-white shadow-lg shadow-primary/20 transition-all hover:scale-[1.02] hover:bg-primary/90 active:scale-[0.98]"
          >
            Open chat
          </Link>
          <Link
            href="/how-it-works"
            className="inline-flex h-12 items-center justify-center rounded-2xl border border-border bg-background px-6 text-sm font-bold transition-all hover:bg-muted"
          >
            Getting Started
          </Link>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="group rounded-3xl border border-border bg-card/40 p-6 transition-colors hover:bg-card/60">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Randi only acts when you say it&apos;s okay.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {callbackBanner && (
          <div
            className={`flex items-center gap-3 rounded-2xl border p-4 text-sm font-medium shadow-sm ${callbackBanner.tone}`}
          >
            <svg
        ...
        {loading ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, index) => (
              <div
                key={index}
                className="h-80 animate-pulse rounded-[2rem] border border-border bg-muted/20"
              />
            ))}
          </div>
        ) : !data ? (

...
export default function FleetPage() {
  ...
  const memoryUsedPercent =
    stats.aggregate.totalMemoryLimit !== '0'
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <p className="text-xs text-muted-foreground">
            {formatBytes(stats.aggregate.totalMemoryUsed)} /{' '}
            {formatBytes(stats.aggregate.totalMemoryLimit)}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4">
      <div className="flex items-start justify-between mb-3">
        ...
        <span
          className={`w-2 h-2 rounded-full ${reportAgo < 60 ? 'bg-green-500' : 'bg-yellow-500'}`}
        ></span>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between text-xs text-muted-foreground pt-2 border-t border-border">
          ...
          <span>{reportAgo < 60 ? `${reportAgo}s ago` : `${Math.floor(reportAgo / 60)}m ago`}</span>
        </div>
      </div>
    </div>
  );

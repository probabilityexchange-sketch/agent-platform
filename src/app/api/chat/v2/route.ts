  ...
  try {
    ...
    if (!isUnmeteredModel(model)) {
      const deduction = await deductForAgentCall(
        auth.userId,
        model,
        `Chat V2: ${message.substring(0, 50)}`,
        sessionId
      );
    ...
    if (composioClient && agent.tools) {
      ...
      const rawTools = (Array.isArray(composioTools) ? composioTools : [composioTools]).map(
        (t: any) => ({
          ...t,
    ...
    ORCHESTRATION_TOOLS.forEach(ot => {
      ...
      (tools as any)[ot.function.name] = tool({
        ...
        execute: async (args: any) => {
          return await executeOrchestrationToolCall(
            auth.userId,
            ot.function.name,
            args,
    ...
    if (existingSession) {
      ...
      stored.reverse().forEach((m: { role: string; content: string }) => {
        ...
        if (!currentSessionId) {
          ...
          try {
            const { text: titleText } = await generateText({
              ...
              system:
                'Generate a concise 3-4 word title for a chat starting with this message. Return ONLY the title text.',
              prompt: message,
            });

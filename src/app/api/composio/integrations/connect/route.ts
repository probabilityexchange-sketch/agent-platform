...
import { getComposioAuthConfigOverride, isComposioToolkitSlug } from '@/lib/composio/integrations';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!authConfigId) {
      ...
      const scopedItems = authConfigs.items.filter(item => item.toolkit.slug === toolkit);

      authConfigId =
        scopedItems.find(item => item.status === 'ENABLED')?.id ?? scopedItems[0]?.id ?? null;
    }

    if (!authConfigId) {
      return NextResponse.json(
        {
          error: 'No auth config found for this toolkit. Create one in Composio dashboard first.',
    ...
    const callbackUrl = `${callbackBaseUrl}/integrations?toolkit=${encodeURIComponent(toolkit)}`;

...
export async function GET() {
  try {
    ...
    const { allowed } = await checkRateLimit(
      `composio-integrations:${auth.userId}`,
      RATE_LIMITS.general
    );
    if (!allowed) {
    ...
    const authConfigMap = new Map<string, string | null>(
      authConfigsPerToolkit.map(e => [e.toolkit, e.selected])
    );
    ...
    const integrations: Integration[] = allToolkits.map((toolkit: any) => {
      ...
      const category = normalizeCategory(
        curated?.category || (toolkit.meta?.categories?.[0]?.name as any)
      );
      const icon = curated?.icon || '🧩';
      const description =
        curated?.description || toolkit.meta?.description || `Integration for ${toolkit.name}`;
    ...
    console.log(
      `[Integrations] Returning ${integrations.length} toolkits for user ${composioUserId}`
    );
    console.log(`[Integrations] Connected accounts breakdown:`);
    for (const [slug, accounts] of accountsByToolkit) {
      console.log(
        `  ${slug}: ${accounts.length} account(s) - ${accounts.map(a => a.status).join(', ')}`
      );
    }
    const activeConnections = integrations.filter(i => i.connected).map(i => i.slug);
    console.log(`[Integrations] ACTIVE connections: ${activeConnections.join(', ')}`);

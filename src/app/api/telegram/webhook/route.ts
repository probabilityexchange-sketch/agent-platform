...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!deduction.success) {
      await sendMessage(
        chatId,
        `⚠️ Insufficient credits to process your request. Required: ${deduction.cost} $RANDI. Please top up on the dashboard.`,
        token
      );
      return NextResponse.json({ ok: true });
    }
              ...
              toolCalls = [
                {
                  ...
                  function: {
                    ...
                    arguments:
                      typeof parsedArgs === 'string' ? parsedArgs : JSON.stringify(parsedArgs),
                  },
                },
              ];
        ...
        const toolResults = await Promise.all(
          toolCalls.map(async tc => {
            ...
            if (tc.type === 'function' && tc.function) {
              ...
              const isOrch = ORCHESTRATION_TOOLS.some(
                (t: any) => t.type === 'function' && t.function?.name === rawFunctionName
              );

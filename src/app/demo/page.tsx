...
const demoSteps = [
      ...
      <div className="bg-card border border-border rounded-xl p-6">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-muted-foreground text-center max-w-xs">
            Randi uses Privy for seamless authentication. Connect with Phantom, MetaMask, or social
            logins.
          </p>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          { name: 'Research Agent', desc: 'Deep research and analysis', model: 'DeepSeek R1' },
          {
            ...
            desc: 'Content, copy, and creative writing',
            model: 'Gemini Flash',
          },
  ...
  },
  {
    ...
    description:
      '$RANDI deposits fuel the deflationary engine. 70% of every deposit is burned forever.',
    content: (
      <div className="bg-card border border-border rounded-xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">
            Deposit Receipt (Simulation)
          </span>
        ...
        <p className="text-[11px] text-muted-foreground text-center">
          Burned tokens are permanently removed from the circulating supply, creating upward price
          pressure.
        </p>
      </div>
    ),
      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-4">
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div
              key={t.tier}
              className={`p-3 rounded-lg border border-border text-center ${t.tier === 'Gold' ? 'bg-primary/5 border-primary/30' : 'bg-muted/30'}`}
            >
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-xl p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              Credits are added to your balance automatically.
            </p>
          </div>
    ...
    content: (
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <p className="text-[11px] text-muted-foreground mt-3 text-center">
          Powered by Composio • 1000+ integrations
        </p>
      </div>
    ),
  },
        ...
        <div className="flex gap-1 mb-8 overflow-x-auto pb-2">
          {demoSteps.map((step, i) => (
            <button
              ...
              className={`flex-1 min-w-[120px] py-2 px-3 text-xs font-medium rounded-lg transition-all whitespace-nowrap ${
                i === activeStep

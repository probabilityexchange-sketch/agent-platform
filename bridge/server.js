...
async function restoreSnapshotToVolume(volumeName, snapshotUrl) {
  ...
  try {
    ...
    const stream = await helperContainer.attach({
      stream: true,
      stdin: true,
      stdout: true,
      stderr: true,
...
async function snapshotVolumeAndUpload(volumeName, userId, agentSlug, storageKey) {
  ...
  try {
    ...
    console.log(
      `[Storage] Snapshot uploaded for volume ${volumeName} (${tarBuffer.length} bytes).`
    );
  } catch (err) {
...
app.post('/spawn-ao', auth, async (req, res) => {
  ...
  if (!/^[a-z0-9-]+$/i.test(project)) {
    return res
      .status(400)
      .json({ error: 'Invalid project name. Only alphanumerics and dashes allowed.' });
  }
  ...
  try {
    ...
    const child = spawn(
      ...
      [
        '-c',
        `cd ${aoPath} && AIDER_MODEL="openrouter/meta-llama/llama-3.3-70b-instruct:free" ao spawn ${project}`,
      ],
      {
        detached: true,
        stdio: 'ignore',
...
app.post('/browse', auth, async (req, res) => {
  ...
  try {
    ...
    const normalizedAction = action === 'screenshot' ? 'screenshot' : 'snapshot';
    let parsedUrl;
    try {
    ...
    const args = [normalizedAction, parsedUrl.toString()];
    ...
    if (wait !== undefined && wait !== null && wait !== '') {
      ...
      if (!Number.isFinite(waitValue) || waitValue < 0) {
        return res.status(400).json({ error: 'Invalid wait value' });
      }
      args.push('--wait', String(waitValue));
    }

    console.log(
      `[Browser] Executing: agent-browser ${args.map(arg => JSON.stringify(arg)).join(' ')}`
    );

    // Execute synchronously for simple request-response.
    // Using argv avoids shell interpolation and blocks command injection.
    ...
    const isNotFound =
      err.message.includes('not found') || err.message.includes('command not found');
...
async function collectAndReportFleetStats() {
  ...
  try {
    ...
    // Report to platform
    const statsPayload = Buffer.from(
      JSON.stringify({
        nodeId: NODE_ID,
        nodeRegion: NODE_REGION,
        totalContainers: containers.length,
        totalCpuPercent: parseFloat(totalCpuPercent.toFixed(2)),
        totalMemoryUsed: Number(totalMemoryUsed),
        totalMemoryLimit: Number(totalMemoryLimit),
        totalNetworkRx: Number(totalNetworkRx),
        totalNetworkTx: Number(totalNetworkTx),
      })
    );
    await fetch(`${PLATFORM_URL}/api/fleet/stats`, {
      ...
      headers: {
        ...
        'Content-Length': statsPayload.length,
    ...
    console.log(
      `[FleetStats] Reported: ${containers.length} containers, CPU: ${totalCpuPercent.toFixed(1)}%, RAM: ${(totalMemoryUsed / 1024 / 1024).toFixed(0)}MB`
    );
  } catch (err) {
    console.error(
      '[FleetStats] Failed to report:',
      err.message,
...
if (SSL_KEY_PATH && SSL_CERT_PATH) {
  ...
  app.listen(PORT, () => {
    ...
    console.warn(
      'WARNING: Running on plain HTTP. Use a reverse proxy or provide SSL_KEY_PATH/SSL_CERT_PATH for production.'
    );
  });
}
